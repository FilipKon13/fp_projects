...
ghc -package-db network-3.2.8.0/dist-newstyle/packagedb/ghc-9.4.7 -package network -isrc -o ${DIST_DIR}/main -odir ${BUILD_DIR} src/Main.hs 

# C
# Compile if we have `rpc.h` header file, otherwise use provided binary (e.g. on `student`)
echo "Building C..."
if [ -f /usr/include/rpc/rpc.h ] ; then
    make -C rpcc -f Makefile.kvstore kvstore_server -B
else

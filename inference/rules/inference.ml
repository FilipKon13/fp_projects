...
let rec unify (t1: typee) (t2: typee) : (subst, string) result =
  match (t1, t2) with
    ...
    | _ -> Error ("Cannot unify " ^ string_of_type t1 ^ " with "
                  ^ string_of_type t2 ^ ": " ^ "types do not match")
...
let type_inference (fc : VarFactory.var_fac) (ctx : context) (expr : expr) : (typee * subst, string) result =
  let rec infer ctx expr = match expr with
    ...
    | EPlus (e0, e1) ->
        let* (tau0, s0) = infer ctx e0 in
        let* (tau1, s1) = infer (apply_subst_context s0 ctx) e1 in
        let* s2 = unify tau0 TInt in
        let* s3 = unify tau1 TInt in
        Ok (TInt, compose_subst s3 (compose_subst s2 (compose_subst s1 s0)))
    | ECons (e0, e1) ->
        let* (tau0, s0) = infer ctx e0 in
        let* (tau1, s1) = infer (apply_subst_context s0 ctx) e1 in
        let* s2 = unify tau1 (TList (apply_subst_type s1 tau0)) in
        Ok (TList (apply_subst_type s2 (apply_subst_type s1 tau0)), compose_subst s2 (compose_subst s1 s0))
    in infer ctx expr
